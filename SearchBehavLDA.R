...
# save console output as doc file
file.create("SearchBehavLDA.doc")
out.File <-file("SearchBehavLDA.doc")
sink(out.File, append = FALSE, type = c("output", "message"))  # initialize sink
...
# Create column for search result
...
search.Data <- cbind(search.Class, (rbind(tp.Data, tn.Data, fn.Data, fp.Data)))
search.Data$PartID <- factor(search.Data$PartID)  # Convert character vectors to 
# factors
search.Data$ExpVer <- factor(search.Data$ExpVer)

# Create column for search result by difficulty treatment
search.DifClass <- factor(paste0(search.Data$ExpVer, search.Class))
search.Data <- cbind(search.DifClass, search.Data)
...
# create and format table showing model performance
m2.pred <-predict(m2.lda, newdata = search.Data[, c(7, 8, 9, 10, 12, 13, 14)])
...
# end sink to output file
sink()
file.info("SearchBehavLDA.doc")
closeAllConnections()
